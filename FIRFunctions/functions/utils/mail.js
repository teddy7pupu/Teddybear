...
exports.sendApprovalMail = function(staff, assignee, leave) {
  ...
  body += `\n假單資訊如下：`
  body += `\n假別：` + leave.type;
  body += `\n請假區間：` + beginDate.format('YYYY-MM-DD hh:mm A') + `到` + endDate.format('YYYY-MM-DD hh:mm A');
  body += `\n請假事由：` + leave.message;
  body += `\n代理人：` + assignee.name;
  body += `\n\n感謝您的合作 :)`;
...
exports.sendFinishMail = function(staff, account, leave) {
  let email = staff.email;
  let accMail = account.email;
  var mailOptions = {
    from: `teddyBear <no-reply-teddybear@pinco.fit>`,
    to: email
  };

  if (accMail) {
    mailOptions["cc"] = accMail
  };
  ...
  mailOptions.text = body;
  return mailTransport.sendMail(mailOptions).then(() => {
    console.log('Mail sent: ', email, ',', accMail);
  });
}
...
exports.sendRejectMail = function(staff, assignee, message, leave) {
  ...
  let beginDate = moment.unix(leave.startTime).add(8, 'h');
  let endDate = moment.unix(leave.endTime).add(8, 'h');
  mailOptions.subject = `[teddyBear] ` + staff.name + `的` + leave.type + `假單申請已被拒絕`;
  let body = `Hi ` + staff.name + `,`;
